$skinBlue = @'
    ...
    <style>
        ...
        .skin-blue .main-sidebar .sidebar>.sidebar-menu>.treeview.active>a,.skin-blue .main-sidebar .sidebar>.treeview>li>a:hover{color:#fff}
...
$oldComment = '    <!-- Inline skin CSS (GitHub Pages fix) -->'

Get-ChildItem "$PSScriptRoot\user\*.html" | ForEach-Object {
    $content = [IO.File]::ReadAllText($_.FullName)
    if ($content.Contains($oldComment) -and -not $content.Contains(".skin-blue .main-header")) {
        $content = $content.Replace($oldComment, $skinBlue)
